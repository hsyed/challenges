...
impl Flags {
    fn from_bytes(left: u8, right: u8) -> Flags {
        Flags([left, right])
    }
...
impl Message {
    pub fn from_bytes(b: &[u8]) -> Result<Message> {
        ...
        let questions = match header.qdcount {
            0 => Vec::new(),
            1 => vec![Question::read(&mut cur).expect("could not parse question")],
            _ => {
    ...
    loop {
        match LabelKind::read(r)? {
            ...
            LabelKind::Data(len) => {
                ...
                qname.push_str(std::str::from_utf8(&label).expect("invalid utf8 label"));
            }
...
impl LabelKind {
    fn read<R: Read>(r: &mut R) -> Result<LabelKind> {
        ...
        if b0[0] == 0 {
            ...
            let mut b1 = [0u8; 1];
            r.read_exact(&mut b1)?;
            Ok(LabelKind::Pointer(u16::from_be_bytes([
                b0[0] & 0x3F,
                b1[0],
            ])))
        } else {
...
mod tests {
    ...
    fn message_query_roundtrip() {
        let sample = [
            112, 27, 1, 32, 0, 1, 0, 0, 0, 0, 0, 1, 3, 119, 119, 119, 6, 103, 111, 111, 103, 108,
            101, 3, 99, 111, 109, 0, 0, 15, 0, 3, 0, 0, 41, 16, 0, 0, 0, 0, 0, 0, 0,
        ];
    ...
    fn message_google_response_roundtrip() {
        let sample = [
            15, 245, 129, 128, 0, 1, 0, 1, 0, 0, 0, 1, 3, 119, 119, 119, 6, 103, 111, 111, 103,
            108, 101, 3, 99, 111, 109, 0, 0, 1, 0, 1, 192, 12, 0, 1, 0, 1, 0, 0, 0, 18, 0, 4, 142,
            250, 179, 228, 0, 0, 41, 2, 0, 0, 0, 0, 0, 0, 0,
        ];
        let message = Message::from_bytes(&sample).unwrap();
        assert_eq!(sample, message.to_udp_packet(None).unwrap().as_slice());
    }
    ...
    fn labelkind_parsing() -> Result<()> {
        ...
        assert_eq!(
            LabelKind::read(&mut Cursor::new(&[0xC0u8, 0x0C]))?,
            LabelKind::Pointer(12)
        );
        Ok(())
    }
